...
using PlutoUI,Images,Distributions,LinearAlgebra,Plots
...
# ╔═╡ 52bf997d-7079-4468-b077-0a5de76e24c5
let 
	B = RGB(0,0,1)
	W = RGB(1,1,1)
	R = RGB(1,0,0)
...
# ╔═╡ c78df405-f94b-4eb2-8130-2d701b25d186
let 
	B = RGB(0,0,1)
	W = RGB(1,1,1)
	R = RGB(1,0,0)
...
# ╔═╡ 7eb301d1-92ed-426a-b8b7-2b422c61eb57
let 
	j = ones(4)
	r = [0 0 1 1 1 1]
	g = [0 0 1 1 0 0]
...
# ╔═╡ 4d3cccf8-2cc5-403e-9bf6-e6a370af9888
let 
	A1 = [1,2]
	A2 = [3,4]
	A =[A1 A2]
...
# ╔═╡ 7404cd33-bf1e-4589-b88b-691ab02e8005
md"""
-- 예제2
"""
...
# ╔═╡ d57a5d02-be07-443c-b2ee-e0e36ce65beb
X = [1 1; 1 2; 5 4]

# ╔═╡ 4e391cbf-782d-4d1b-a7d4-85758454c84d
U,d,V = svd(X)

# ╔═╡ 1b4769f6-41e0-420d-9392-4b0ac4a67572
U*Diagonal(d)*V'

# ╔═╡ e4074014-5201-4870-9aa6-5dc692460eae
V*V'

# ╔═╡ 31453994-5eca-44d6-9496-04b4081e4394
V'V

# ╔═╡ 114a0f1f-e7aa-43ed-b0e2-ab0adb47436b
U*U'

# ╔═╡ 89b63250-cf07-4f6c-a292-33635dc7f186
U'U
...
md"""
-- 예제2: $X \sim {\cal N}\big({\boldsymbol \mu}, \boldsymbol{\Sigma} \big)$
"""

# ╔═╡ 4b9d1961-0d6e-4b93-b635-dc8b71fa2a9f
let
	μ = [1,5,3] 
...
# ╔═╡ 641e0058-23ef-4a2e-b72b-61ee80d9a4c9
md"""
-- 예제3
"""
...
let 
	...
	k = 3
	R = Ur[:,1:k] * Diagonal(r[1:k])  * Vr[:,1:k]'
	G = Ug[:,1:k] * Diagonal(g[1:k])  * Vg[:,1:k]'
	B = Ub[:,1:k] * Diagonal(b[1:k])  * Vb[:,1:k]'
	#colorview(RGB, stack([R,G,B],dims=1))
	colorview(RGB, R,G,B)
end

# ╔═╡ 8d91c59e-4cb4-4d3d-8fd4-203c3081e88f
md"""
### D. 하니
"""

# ╔═╡ 758b4f57-baed-48c6-919e-3dd52efa1080
...
let 
	(Ur,r,Vr), (Ug,g,Vg), (Ub,b,Vb)= eachslice(channelview(hani),dims=1) .|> svd
	R = Ur[:,1:k] *Diagonal(r[1:k]) * Vr[:,1:k]'
	G = Ug[:,1:k] *Diagonal(g[1:k]) * Vg[:,1:k]'
	B = Ub[:,1:k] *Diagonal(b[1:k]) * Vb[:,1:k]'
	println("ratio=$((4032*k + k +k*3024) / (4032*3024))")
	hani2 = colorview(RGB,R,G,B)
	[hani hani2]
end 

# ╔═╡ abd269c4-7c43-4f26-a5cc-8b7940d7daa4
md"""
-- SVD를 이용하면 이미지를 압축하여 효율적으로 전달할 수 있음. 

- SVD는 행렬을 분해하는 알고리즘이지, 이미지를 압축하는 알고리즘은 아님. (거기에 이용할 뿐)
- SVD는 훨씬 다재다능함. 
"""
