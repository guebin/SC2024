...
!!! info "이론: 스펙트럼 정리 (Spectral Theorem)" 
	임의의 ``n \times n`` 행렬 ``{\bf A}``가 실대칭행렬이라면 (1) 모든 고유값이 실수이고 (2) 직교대각화 가능하다. 즉 ${\bf A}$를 아래와 같이 표현할 수 있다. 

	${\bf A} = {\bf \Psi} {\bf \Lambda}{\bf \Psi}^\top$

	여기에서 ${\bf \Psi}$는 고유벡터행렬이고, ${\bf \Lambda}$는 고유값행렬이다. 
"""
...
md"""
-- 예제2. 직교대각화 가능한 행렬 ${\bf A}$의 모든 고유값이 0이 아니라면, ${\bf A}$는 full-rank matrix임을 보여라. (즉 ${\bf A}$의 역행렬이 존재함을 보여라.)
"""
...
# ╔═╡ 3a161215-0ba1-4a2d-ab54-f571b03cfbe2
md"""
-- 예제4: 아래의 예제는 대칭행렬이지만 고유값이 실수가 아니다. 또한 이 행렬은 대각화가능하지도 않다. 이를 체크하라.
"""

# ╔═╡ 7c2e0d07-2029-441b-9ed0-0eadca435407
let 
	A = [2im 1
		 1   0]
end
